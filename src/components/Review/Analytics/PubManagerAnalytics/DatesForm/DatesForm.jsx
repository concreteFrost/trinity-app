...
export const DatesForm = () => {
  ...
  function _GetDoorstaffAnalytics() {
    GetDoorstaffRecentAPI(user, analytics.dateFrom, analytics.dateTo, "S").then(
      (res) => {
  ...
  function _GetCostsAnalytics() {
    GetDoorstaffRecentAPI(user, analytics.dateFrom, analytics.dateTo, "A").then(
      (res) => {
      ...
      <form onSubmit={Submit}>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              dispatch({ type: "SET_DATE_TO_ANALYTICS", data: e.target.value });
            }}
          />
        </div>
