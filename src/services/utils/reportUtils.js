...
import {
  GetAreaManagerAnalyticsCosts,
  GetAreaManagerAnalyticsDoorstaff,
...
export function GeneratePDF(token, system, activityId, dispatch) {
  GetTimesheetDataAPI(token, system, activityId)
    .then((res) => {
      const receiptTitle =
...
export function GetAreaManagerAnalytics(system, token, analytics, dispatch) {
  const requests = analytics.locations
    ...
    .map((location) => {
      return GetSearchedDataAPI(system, token, location.id, analytics).then(
        (res) => {
