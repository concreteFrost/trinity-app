...
import {
  GetDisputedActivityAPI,
  SendBackDisputedAPI,
  ViewDisputedNoteAPI,
} from "services/disputedApi";
...
export const Disputed = (props) => {
  ...
  function viewNote(e) {
    ViewDisputedNoteAPI(token, props.system, e).then((res) => {
      dispatch(
        ShowModalMessage(res.data.record[res.data.record.length - 1].name)
      );
    });
  }
  ...
  async function DisputeBack() {
    ...
    await GetDisputedActivityAPI(token, props.system).then((res) => {
      switch (props.system) {
        ...
        case "S":
          dispatch(
            DoorstaffActions.GetDisputedDoorstaff(res.data.reportRecord)
          );
          break;
      }
    });
