...
export const Current = (props) => {
  ...
  const doorstaff = useSelector((state) => state.doorstaffReducer.current);

  const [isSignOffModalVisible, setIsSignOffModalVisible] = useState(false);

  const errorMessage = useSelector(
    (state) => state.doorstaffReducer.errorMessage
  );
  ...
  return (
    <div className={s.container}>
      {errorMessage.length > 0 && !isSignOffModalVisible ? (
      ...
      {props.isVisible && doorstaff.length > 0 ? (
        <CurrentFooter
          ShowSignOffSelectedModal={ShowSignOffSelectedModal}
        ></CurrentFooter>
      ) : null}

      {props.isVisible && isSignOffModalVisible ? (
        <CurrentModal
          doorstaff={doorstaff}
          token={token}
          setIsSignOffModalVisible={setIsSignOffModalVisible}
