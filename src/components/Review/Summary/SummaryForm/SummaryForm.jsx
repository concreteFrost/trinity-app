...
export const SummaryForm = () => {
  ...
  async function SubmitForm(e) {
    e.preventDefault();
    dispatch(ShowLoader());
    await GetSummaryReviewAPI(token, date, "D")
      .then((res) => {
        if (isErrorStatus(res)) {
          dispatch(GetBadResponse("get summary review error", res, "review"));
        }
        dispatch(
          DoorstaffActions.GetDoorstaffSummaryDaily(res.data.summaryRecords)
        );
    ...
    await GetSummaryReviewAPI(token, date, "W")
      .then((res) => {
        if (isErrorStatus(res)) {
          dispatch(GetBadResponse("get summary review error", res, "review"));
        }
        ...
        dispatch(
          DoorstaffActions.GetDoorstaffSummaryWeekly(res.data.summaryRecords)
        );
        dispatch(
          ActivityActions.GetCostsSummaryWeekly(res.data.summaryRecords)
        );
      })
      .catch((e) => {
        dispatch(GetBadResponse("get summary review error", e, "review"));
      });
    await GetSummaryReviewAPI(token, date, "M")
      .then((res) => {
        if (isErrorStatus(res)) {
          dispatch(GetBadResponse("get summary review error", res, "review"));
        }
        ...
        dispatch(
          DoorstaffActions.GetDoorstaffSummaryMonthly(res.data.summaryRecords)
        );
        dispatch(
          ActivityActions.GetCostsSummaryMonthly(res.data.summaryRecords)
        );
      })
      .catch((e) => {
        dispatch(GetBadResponse("get summary review error", e, "review"));
