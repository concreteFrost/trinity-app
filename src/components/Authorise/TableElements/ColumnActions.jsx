...
export const ColumnActions = (props) => {
  ...
  function toggleMoreActions(e) {
    ...
    path.endsWith("doorstaff")
      ? dispatch({ type: "TOGGLE_MORE_ACTIONS_FOR_DOORSTAFF", data: e })
  ...
  function showRecallIfno() {
    ...
    if (props.system === "S") {
      message = "This action will reset the shift";
    } else {
  ...
  return (
    <div className={s.actions_wrapper}>
      <div className={s.actions}>
        ...
        <button
          onClick={() => toggleMoreActions(props.row.original.activityId)}
        >
          MORE
        </button>
      </div>
      {props.row.original.moreActionsVisible ? (
        <div className={s.additional_actions}>
          <div className={s.additional_action_item}>
            <button
              className={s.recall}
              onClick={() => showRecallModal(props.row.original.activityId)}
            >
              {props.system === "S" ? "RECALL" : "DELETE"}
            </button>
            <button className={s.question} onClick={showRecallIfno}>
              ?
            </button>
          </div>
