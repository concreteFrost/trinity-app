...
import {
  GetAreaManagerAnalyticsLocations,
  ToggleAreaManagerAnalyticsLocations,
} from "redux/actions/analyticsActions";

export const LocationList = (props) => {
  const dispatch = useDispatch();
  const locationsList = useSelector(
    (state) => state.areaManagerAnalyticsReducer.locations
  );
  ...
  return (
    <div className={s.location}>
      ...
      <div className={s.dropdown}>
        <button
          onClick={props.toggleShowLocations}
          className={s.dropdown_button}
        >
          {props.showDropdown ? "Close" : "Show"}
        </button>
        {props.showDropdown ? (
                  ...
                  return (
                    <label key={e.id}>
                      <input
                        ...
                        checked={e.isChecked}
                        onChange={() => {
                          dispatch(ToggleAreaManagerAnalyticsLocations(e.id));
                        }}
                      />
                      {e.name}{" "}
                    </label>
